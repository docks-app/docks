...
describe Docks::PostProcessors::CleanUpVariantsAndStates do
  ...
  it 'sets appropriate defaults' do
    ...
    result_state = subject.post_process([{ name: 'tab', type: Docks::Types::Symbol::COMPONENT, states: [{ name: 'tab--is-active', type: Docks::Types::Symbol::STATE }] }]).first[:states].first
    default.each do |k, v|
  ...
  it 'prepends the base class to the state/ variant name' do
    result_state = subject.post_process([{ name: 'tab', type: Docks::Types::Symbol::COMPONENT, states: [{ name: '--is-active', type: Docks::Types::Symbol::STATE }] }]).first[:states].first
  ...
  it 'prepends the base class to the activate_with, include_with, and precludes details' do
    result_state = subject.post_process([{ name: 'tab', type: Docks::Types::Symbol::COMPONENT, states: [{ name: '--is-active', type: Docks::Types::Symbol::STATE, precludes: ['--is-inactive'], activate_with: ['--visible'], include_with: ['--visible'] }] }]).first[:states].first
